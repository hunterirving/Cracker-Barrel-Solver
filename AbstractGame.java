...
public abstract class AbstractGame<StateType extends State>
{
   ...
   // call a method addNewState with each of the children of the
   // currentState.  The addNewState method adds the child if it
   // is not a duplicate, and returns true.  If the state is a 
   // duplicate, returns false. Remember to make a NEW state for
   // each new state.  Do not overwrite the currentState. 
   //
   // for each child off the currentStste 
   //    addNewState(child)
   ...
   // return true if the currentState is a goalState 

   abstract public boolean goalState();

   // set the currentState to the initialState
